...
using namespace std;
static std::string speaker,appid,pitch;//需要launch 配置文件传入发音人名称
//static int speed,volume,rdn;//语速，音量，音调，数字处理方式
bool pubFlag = true;
...
*/
void makeTextToWav( const char* text , const char* filename , const char* speaker,const char* pitch,const char* appid)
{
	char session_begin_params[120];
	char login_params[120];

	int         ret                  = MSP_SUCCESS;
	//const char* login_params         = "appid = 5aaa223d, work_dir = .";//登录参数,appid与msc库绑定,请勿随意改动
	...
	*/
	...
	sprintf(session_begin_params, "voice_name = %s, text_encoding = utf8, sample_rate = 16000, speed = %d, volume = 80, pitch = %s, rdn = 0",speaker,50,pitch);
	//ROS_ERROR("pitch is %s",pitch);
...
void mplayer(string result)
{
	if(result.compare("故事目录")==0){
		system("play src/voice_system/play_data/story/0故事目录.wav");
		pubFlag = false;
	}
	if(result.compare("故事：丑小鸭")==0){
		system("play src/voice_system/play_data/story/1丑小鸭.mp3");
		pubFlag = false;
	}
	if(result.compare("故事：圣甲虫")==0){
		system("play src/voice_system/play_data/story/2圣甲虫.mp3");
		pubFlag = false;
	}
	if(result.compare("故事：幸运的套鞋")==0){
		system("play src/voice_system/play_data/story/3幸运的套鞋.mp3");
		pubFlag = false;
	}
	if(result.compare("故事：萤火虫")==0){
		system("play src/voice_system/play_data/story/4萤火虫.mp3");
		pubFlag = false;
	}
	if(result.compare("儿歌：小恐龙")==0){
		system("play src/voice_system/play_data/story/5小恐龙.mp4");
		pubFlag = false;
	}
	if(result.compare("儿歌：变")==0){
		system("play src/voice_system/play_data/story/6变.mp4");
		pubFlag = false;
	}
	if(result.compare("儿歌：胖小猪")==0){
		system("play src/voice_system/play_data/story/7胖小猪.mp4");
		pubFlag = false;
	}
	if(result.compare("music")==0){
		system("play src/voice_system/play_data/music/We_Found_Love.mp3");
		pubFlag = false;
	}

	if(result.compare("finishstep0")==0){
		system("play src/voice_system/play_data/register/step_0.wav");
		sleep(2);
		system("play src/voice_system/play_data/register/step_1.wav");
		pubFlag = false;
	}
	if(result.compare("finishstep1")==0){
		system("play src/voice_system/play_data/register/step_2.wav");
		pubFlag = false;
	}
	if(result.compare("finishstep2")==0){
		system("play src/voice_system/play_data/register/step_3.wav");
		pubFlag = false;
	}
	if(result.compare("finishstep3")==0){
		system("play src/voice_system/play_data/register/step_4.wav");
		pubFlag = false;
	}
}
...
void topicCallback(const std_msgs::String::ConstPtr& msg)
{
	...
	mplayer(msg->data);
	if(pubFlag){
		makeTextToWav( msg->data.c_str() , "/root/catkin_ws/src/voice_system/text.wav",speaker.c_str(),pitch.c_str(),appid.c_str());
		system("play /root/catkin_ws/src/voice_system/text.wav");	
	}
}
